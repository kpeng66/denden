...
const MathGame: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const wsConnection = new WebSocket(
      `ws://192.168.1.67:8000/ws/mathgame/${room_code}`
    );
    setWs(wsConnection);
    ...
    wsConnection.onmessage = (event) => {
      ...
      console.log(
        `Websocket message received at ${new Date().toISOString()}:`,
        message
      );
  ...
  const handleWebsocketMessage = (message: any) => {
    switch (message.type) {
      case "game.countdown":
        console.log(
          `Countdown message received at ${new Date().toISOString()}`
        );
      ...
      case "game.timer":
        console.log(
          `Game timer is ${message.timer} at ${new Date().toISOString()}`
        );
  ...
  const fetchNewEquation = async () => {
    const response = await axios.get(
      "http://192.168.1.67:8000/api/get-new-equation"
    );
  ...
  const handleNumberPress = (number: number) => {
    ...
    if (
      ...
      newInputValue[0] !== correctAnswer[0]
    ) {
  ...
  const handleAnswer = async (userInput?: string) => {
    ...
    try {
      const response = await axios.post(
        "http://192.168.1.67:8000/api/check-answer",
        {
          ...
          <div className={styles.timer}>
            <h4>
              {preGameCountdown !== null
                ? `Game Starts In: ${
                    preGameCountdown > 0 ? preGameCountdown : "Now"
                  }s`
                : `Time Left: ${timer}s`}
            </h4>
          </div>
